...
public class PatternPreviewWidget extends WidgetGroup {
    ...
    public void updateScreen() {
        super.updateScreen();
        // GTNEcore patch 层与 fork 版的差异：此处省略 EMI/REI 屏幕分支（fork 版有
        // RecipeScreen/AbstractDisplayViewingScreen instanceof 判断）——patch 编译
        // classpath 无 REI/EMI 依赖（同 AE2/TOP 软依赖裁剪惯例），整合包只用 JEI。
    }
    ...
    private MBPattern initializePattern(MultiblockShapeInfo shapeInfo, HashSet<ItemStackKey> blockDrops) {
        ...
        if (controllerBase != null) {
            ...
            // GTNEcore patch：成型检测失败时 matchContext 里没有 "predicates"（get 返回 null），
            // 点击预览方块时 onPosSelected 会 NPE 崩溃（"无法在JEI预览"根因之一）。
            // 兜底：保持空 map，onPosSelected 里 predicate==null 静默返回。
            var ctxPredicates = controllerBase.getMultiblockState().getMatchContext().get("predicates");
            if (ctxPredicates instanceof Map<?, ?> map) {
                //noinspection unchecked
                predicateMap = (Map<BlockPos, TraceabilityPredicate>) map;
            }
        }
