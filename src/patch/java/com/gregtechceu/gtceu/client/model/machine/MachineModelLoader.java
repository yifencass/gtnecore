...
public class MachineModelLoader implements IGeometryLoader<UnbakedMachineModel> {
    ...
    public @Nullable UnbakedMachineModel read(JsonObject json,
        ...
        try {
            ...
            // MekaTech patch: RotationState.NONE 机器（无 facing 属性）的 blockstate 只有空变体 (#)，
            // 需映射到默认渲染状态，否则 "missing model for variant: xxx#" → 模型加载失败紫黑
            if (!possibleStates.isEmpty()) {
                modelsToStates.put(new ModelResourceLocation(machineId, ""), possibleStates.get(0));
            }
            ...
            // MekaTech patch: MachineDefinition.defaultRenderState 是 setupStateDefinition 里
            // any().setValue(...) 产生的新实例，不属于 stateDefinition.getPossibleStates()。
            // modelsByState（bake 阶段）是 IdentityHashMap（MachineRenderState 无 equals/hashCode），
            // 按实例查找必然 miss → MachineModel.getParticleIcon() 对这类机器 NPE。
            // 这里把 defaultRenderState 也映射进 statesToModels：优先复用属性值等价的
            // possibleStates 实例所对应的模型，让挖掘粒子的贴图正确。
            MachineRenderState defaultState = definition.defaultRenderState();
            if (defaultState != null && !statesToModels.containsKey(defaultState)) {
                UnbakedModel defaultModel = possibleStates.stream()
                        .filter(state -> state.getValues().equals(defaultState.getValues()))
                        .map(statesToModels::get)
                        .filter(Objects::nonNull)
                        .findFirst()
                        .orElse(MISSING_MARKER);
                statesToModels.put(defaultState, defaultModel);
            }
        }
